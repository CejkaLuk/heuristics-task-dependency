...
class CPMActivitiesLoader():
    """Loader of activities for CriticalPathMethod (CPM) from a file."""

    ## Public methods
    @staticmethod
    def get_cpm_activities(acts_file_path: str) -> List[Activity]:
        ...
        with open(acts_file_path, encoding="utf-8") as file:
            ...
            for line in lines:
                act = CPMActivitiesLoader._get_activity_from_line(line)
                CPMActivitiesLoader._check_if_duplicate_activity(act, activities)
    ...
    def _get_activity_from_line(line: List[str]) -> Activity:
        ...
        CPMActivitiesLoader._validate_activity_line(line)
    ...
    @staticmethod
    def _check_if_duplicate_activity(act: Activity, acts: List[Activity]):
        """Checks if the activity is already present in activities."""
        if act.id in [a.id for a in acts]:
