...
class SerialMethodTestSuite(unittest.TestCase):
    ...
    @params(# Problem 1
            (ProblemsPaths.problem_1_dir, 7, 24),
            # Problem 2
    ...
    def test_to_json(self, problem_dir: str, r_max: int):
        ...
        cpm_acts_schedule = self._get_json_from_file(cpm_json_file)
        serial_method_acts_schedule = self._get_json_from_file(serial_json_file)

        correct_cpm_acts_schedule = self._get_json_from_file(correct_cpm_acts_schedule_file)
        correct_serial_method_acts_schedule = self._get_json_from_file(
    ...
    def get_correct_activities(cpm_correct_acts_file: str,
        ...
        with open(serial_method_correct_acts_file, encoding='utf-8') as sol:
            ...
            for line in lines:
                ...
                act = SerialMethodTestSuite._get_act_by_id(id, activities)
    ...
    @staticmethod
    def _get_act_by_id(id: ID, activities: List[Activity]) -> Activity:
        return next(filter(lambda act: act.id == id, activities))

    @staticmethod
    def _get_json_from_file(file_path: str):
